...
const groupBlogPostsByYear = (posts: Array<PartialBlog>) => {
  return posts.reduce<Record<number, Array<PartialBlog>>>((acc, post) => {
...
export const BlogPostsList = async () => {
  const posts = await getAllPosts();
  const blogPostsByYear = groupBlogPostsByYear(
    posts.map((it) => ({ ...it, keywords: undefined, content: undefined })),
  );
  return (
